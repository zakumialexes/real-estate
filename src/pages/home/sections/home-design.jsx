...
export function PopperSelect({ type, data, propertyType, isNumber }) {
    ...
    return (
        <Box ref={anchorEl} sx={style.wrapper}>
            ...
            >
                <Typography>
                    {select ? select?.value : type} {select && isNumber && type}
                </Typography>
                {open ? (
                    <ArrowDropUpIcon fontSize="small" sx={{ color: "black" }} />
                ) : (
                    <ArrowDropDownIcon fontSize="small" sx={{ color: "black" }} />
                )}
            </Stack>

            <Popper open={open} anchorEl={anchorEl.current} sx={{ position: "relative" }}>
    ...
    return (
        <Container sx={container}>
            <Box sx={{ margin: "15px" }}>
                ...
                >
                    <Typography variant="body1" color="#006c70" sx={{ fontWeight: "bold" }}>
                        ${value[0]}
                    </Typography>
    ...
    return (
        <Box ref={price} sx={{ position: "relative", cursor: "pointer", flexGrow: 1 }}>
            <Stack
                ...
                sx={selector}
            >
                <Typography sx={{ color: display ? "black" : "silver" }}>
                    {display ? `$${display[0]}-$${display[1]}` : "Price"}
                </Typography>
                {open ? (
                    <ArrowDropUpIcon fontSize="small" color="black" />
                ) : (
...
const Amenities = ({ hide, screen }) => {
    ...
    const yearBuilt = [2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021, 2022, 2023]
    ...
    const CheckBox = ({ label }) => {
        return (
            <Grid item md={2} sm={3} xs={4} size="small">
                <FormControlLabel control={<Checkbox />} label={label} labelPlacement={"end"} />
            </Grid>
        )
    }
    return (
        <Box sx={wrapper}>
            <Box sx={{ padding: "30px", width: "fit-content" }}>
                ...
                >
                    <Typography variant="h6" color="#006c70" sx={{ fontWeight: "bold" }}>
                        Amenities
                    </Typography>
                    <Button variant="contained" sx={{ width: "fit-content" }} onClick={hide}>
                        HIDE
                    </Button>
                </Stack>
                <Grid
...
const SearchBar = () => {
    ...
    return (
        <Stack
            ...
            gap={2}
            ref={smallerScreen ? null : advance}
        >
            <TextField inputProps={inputField} margin="none" placeholder="Enter Keywords..." sx={{ flexGrow: 1 }} />

            <PopperSelect type={"Property Type"} data={propertyType} propertyType={true} />
            ...
            <Advanced />
            <Button variant="contained" color="primary" sx={btn}>
                Search
            </Button>
        </Stack>
    )
            ...
            <Container className={style.Container}>
                ...
                <Box className={style.body}>
                    <Stack direction="row" gap={3}>
                        <Box
                            className={isBuy ? style.toggleBtnSelected : style.toggleBtn}
                            onClick={() => {
                                setIsBuy((pre) => (pre ? pre : !pre))
                            }}
                        >
                            Buy
                        </Box>
                    ...
                    <Box sx={{ marginTop: "30px" }}>{isBuy ? <SearchBar /> : <SearchBar />}</Box>
                </Box>
            </Container>
        </Box>
    )
