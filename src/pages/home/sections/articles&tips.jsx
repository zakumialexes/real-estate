  ...
  return (
    <Card className={style.wrapper}>
        <Box className={style.imageWrapper}>
            <CardMedia component="img" image={image} alt="image" sx={{zIndex: 0, objectFit: "cover", width: "100%"}}/>
        </Box>
        <CardContent>
                <Typography variant='subtitle2' sx={{color: "#ff5a5f", marginBottom: "5px"}}>{type}</Typography>
                <Typography variant='h6' sx={{fontWeight: "bold", color: "#484848", marginBottom: "5px"}}>{title}</Typography>
        </CardContent>
        <Divider/>
...
const ArticleTips = () => {
  ...
  return (
    <Wrapper color={"#f7f7f7"} title={title} content={content}>
      ...
      {articles?.map((article, index) => <ArticleCard key={index} image={article.image} avatar={article.authorImg} author={article.author} date={article.date} title={article.title} type={article.topic} />)}
      </Stack>
    </Wrapper>
  )
}
