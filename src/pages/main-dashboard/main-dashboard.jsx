...
const MainDashboard = () => {
    return (
        <Box width="100%" sx={{ backgroundColor: "#f7f7f7" }} p={{ xs: 2, lg: 5 }}>
            <Box mb={5} py={2}>
                <Typography variant="h5" fontSize="30px" fontWeight="bold" style={{ lineHeight: "2.5rem" }}>
                    Howdy, Ali!
                </Typography>
                ...
                {countData?.map((data, index) => (
                    ...
                    >
                        <Stack direction="column">
                            <CountUp end={data.number} duration={3.5} style={{ fontSize: "36px" }} />
                        ...
                        >
                            <FontAwesomeIcon icon={data.icon} color={data.color} fontSize="25px" />
                        </Stack>
                    </Stack>
                ))}
            </div>
            <Stack direction={{ xs: "column", lg: "row" }} alignItems={{ xs: "center", lg: "flex-start" }} spacing={5}>
                <LeftSide />
                <RightSide />
            </Stack>
        </Box>
