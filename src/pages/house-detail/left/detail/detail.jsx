...
import { faAngleDown, faCheck, faDownload } from "@fortawesome/free-solid-svg-icons"
import { PropertyDetails, AddtionalDetails, Description, Features } from "../../data"
        ...
        <>
            <div id="details" className={detailStyle.container}>
                ...
                >
                    ...
                    <FontAwesomeIcon
                        icon={faAngleDown}
                        className={`${detailStyle.icon} ${show ? detailStyle.iconActive : ""}`}
                    />
                </Stack>
            </div>
                ...
                <div className={detailStyle.propertyDetails}>
                    {PropertyDetails?.map((data, index) => (
                        <div key={index} className={detailStyle.propertyDetailsItem}>
                            <span>{data.item} :</span> <span style={{ fontWeight: "bold" }}>{data.value}</span>
                        </div>
                    ))}
                </div>
            </div>
                ...
                <div className={detailStyle.addtionalDetails}>
                    <Stack direction="row" alignItems="center" spacing={2}>
                        <div className={detailStyle.iconContainer}>
                            <FontAwesomeIcon icon={faFileWord} color="#ff5a5f" style={{ fontSize: "30px" }} />
                        </div>
                        <Stack direction="row" alignItems="center" justifyContent="center" spacing={2}>
                            <FontAwesomeIcon icon={faDownload} color="#ff5a5f" />
                            <span style={{ fontWeight: "bold", fontSize: "16px" }}>Demo Word Document</span>
                        </Stack>
                    </Stack>
                    <Stack direction="row" alignItems="center" spacing={2}>
                        <div className={detailStyle.iconContainer}>
                            <FontAwesomeIcon icon={faFilePdf} color="#ff5a5f" style={{ fontSize: "30px" }} />
                        </div>
                        <Stack direction="row" alignItems="center" justifyContent="center" spacing={2}>
                            <FontAwesomeIcon icon={faDownload} color="#ff5a5f" />
                            <span style={{ fontWeight: "bold", fontSize: "16px" }}>Demo Pdf Document</span>
                        </Stack>
                    </Stack>
                </div>
            </div>
            <div id="features" className={detailStyle.container} style={{ marginTop: "30px" }}>
                <Typography fontSize="18px" fontWeight="bold" mb={2}>
                    Features
                </Typography>
                <div className={detailStyle.propertyDetails}>
                    {Features?.map((data, index) => (
                        <Stack key={index} direction="row" alignItems="center" spacing={2} pb={1} pt={1}>
                            <FontAwesomeIcon icon={faCheck} color="#ff5a5f" style={{ fontSize: "20px" }} />
                            <span>{data.name}</span>
                        </Stack>
                    ))}
                </div>
